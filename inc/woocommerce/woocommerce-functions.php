...
function denifire_woocommerce_related_products_args( $args ) {
	global $sitepress; // Access the WPML plugin's global object
	...
	$defaults = array(
		'posts_per_page' => 3,
		'columns'        => 3,
		'lang'           => $current_language, // Add the language parameter
	);
...
function add_contact_form() {
	...
	if ( get_locale() == 'en_GB' || get_locale() == 'en_US' ) {
		echo do_shortcode( '[contact-form-7 id="ea54249" title="Contact form"]' );
	} else {
		echo do_shortcode( '[contact-form-7 id="ea54249" title="Контактна форма"]' );
	}
	?>
		</section>
	<?php
...
function add_disclaimer() {
	?>
		...
		<?php
		if ( get_field( 'disclaimer_text', 'option' ) ) {
			the_field( 'disclaimer_text', 'option' );
			// echo __( 'На пазара в Република България се предлагат и предоставят единствено продукти , които са одобрени  и разрешени за продажба съгласно действащите наредби в Република България. ', 'denifire' );
		}
		?>
		</p>
		</div>
